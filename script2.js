...
class Cube {
  ...
  init(unlockeds) {
    ...
    this.updateColors( this.game.themes.getColors(), unlockeds );
  ...
  updateColors( colors, unlockeds=[] ) {

    if ( typeof this.pieces !== 'object' && typeof this.edges !== 'object' ) return;

    //AP
    if(!this.lockedColors){
      this.edges.sort((a, b) => a.name.localeCompare(b.name));
      for (let i = 0; i < this.edges.length; i++) {
        if(unlockeds && unlockeds.includes(i)){
          this.edges[i].name = this.edges[i].name + 'O' + '-' + (i + 1);
        }else{
          this.edges[i].name = this.edges[i].name + 'X' + '-' + (i + 1);
        }
      }
      this.lockedColors = true;
      console.log('locked colors!', this.edges.map(edge => edge.name));
    }
...
class Controls {
  ...
  checkIsSolved() {
    ...
    for (let i = 0; i < sideKeys.length; i++) {
      ...
      const colorCounts = sides[side].reduce((acc, name) => {
        if (name.charAt(1) === 'O') {
      ...
      // Find the maximum count (most common color)
      const maxCount = Object.values(colorCounts).length > 0 ? Math.max(...Object.values(colorCounts)) : 0;
      correctCount += maxCount;
    }

    const maxMaxPossible = 6 * (this.game.cube.size * this.game.cube.size);

    // Update the correctness text
    this.game.dom.texts.correctness.textContent = `${correctCount}/${maxPossible}/${maxMaxPossible}`;
    ...
    if(correctCount === maxPossible && maxPossible < maxMaxPossible){
      const allEdges = [].concat(...Object.values(sides));
      console.log(allEdges)
      const found = false;
      for (let i = 0; i < allEdges.length; i++) {
        if (allEdges[i].charAt(1) === 'X') {
          allEdges[i] = allEdges[i].charAt(0) + 'O' + allEdges[i].substring(2);
          this.game.cube.updateColors(this.game.themes.getColors());
          console.log(`turning ${i} into O`)
          this.checkIsSolved();
          found = true;
          break;
        }
      ...
      if(!found){
        console.log('All stickers are correct but no X found??')
      }
    }
...
class Preferences {
  ...
  init() {
    ...
    this.ranges = {
      ...
      //   range: [ 2, 5 ],  // THIS IS WHERE THE MAX SIZE IS SET
      //   step: 1,
      //   onUpdate: value => {
...
window.game = new Game(2, [9, 14]);

document.addEventListener('keydown', event => {
  if (event.key.toUpperCase() === 'P') {
    window.game.unlockSide(10);
  }
});