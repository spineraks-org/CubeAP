class IconsLoader {
    ...
    loadIcon() {
        ...
        this.#svgTag.setAttribute('class', this.options.className);

        if (this.options.styles) this.#addStyles();
        if (this.options.convert) this.#convertAllIcons();
    ...
    #convertAllIcons() {
        document.querySelectorAll(this.options.tagName).forEach((icon) => {
            this.#convertIcon(icon);
        });
    }

    #convertIcon(icon) {
        const svgData = this.options.icons[icon.attributes[0].localName];
    ...
    #addStyles() {
        ...
        style.innerHTML = `.${this.options.className} { display: inline-block; font-size: inherit; overflow: visible; vertical-align: -0.125em; preserveAspectRatio: none; }`;
        document.head.appendChild(style);
    }
}

export default new IconsLoader({
    icons: {
        settings: {
            viewbox: '0 0 512 512',
            content:
